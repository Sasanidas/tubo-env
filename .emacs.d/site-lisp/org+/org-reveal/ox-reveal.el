...
(defun org-reveal--check-resource ()
  "Check js/slides.js exist or not, if not exist, re-fetch resource."
  (let* ((package (concat yc/emacs-tools-dir "reveal.js-3.7.0.7z"))
         (targetdir (concat default-directory "reveal.js"))
         (targetbase (file-name-directory targetdir))
         (basename (file-name-nondirectory package))
         (extname (file-name-extension basename))
         (version (if (string-match
                       (format (rx "reveal.js-"
                                   (group (+ (or digit "."))) ".%s") extname)
                       basename)
                      (match-string 1 basename)
                    (error "oops, match fails...")))
         )


    (when (or (not (file-exists-p targetdir)) ;; target does not exists
              (not (file-exists-p (concat targetdir "/js/reveal.js")))
              (with-temp-buffer
                (insert-file (concat targetdir "/js/reveal.js"))
                (goto-char (point-min))
                ...
                (if (search-forward-regexp
                  ...
                  t)))
      (let ((cmd (format "7z x %s -o%s  && mv %s %s"
                         package targetbase
                         (concat targetbase "/" (file-name-sans-extension basename))
                         targetdir)) )
